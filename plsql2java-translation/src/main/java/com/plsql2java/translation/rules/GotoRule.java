...
@Component
public class GotoRule implements TranslationRule {

    @Override
    public ConstructType getConstructType() {
        return ConstructType.GOTO;
    }

    @Override
    public TranslationOutcome apply(AstNode node, TranslationContext context) {
        return TranslationOutcome.flagged(
                "GOTO has no Java equivalent",
                "Refactor control flow to eliminate GOTO before migration",
                30
        );
    }
}
